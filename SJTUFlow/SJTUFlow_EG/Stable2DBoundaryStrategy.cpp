...
void Stable2DBoundaryStrategy::setBoundary(int size, BoundaryType type, float *boundary)
{
	int i;
	int N = size;

	for ( i=1 ; i<=N ; i++ ) {
		boundary[IX(0  ,i  )] = type==BoundaryTypeU ? -boundary[IX(1,i)] : boundary[IX(1,i)]; // horizontal
		boundary[IX(N+1,i  )] = type==BoundaryTypeU ? -boundary[IX(N,i)] : boundary[IX(N,i)];
		boundary[IX(i  ,0  )] = type==BoundaryTypeV ? -boundary[IX(i,1)] : boundary[IX(i,1)]; // vertical
		boundary[IX(i  ,N+1)] = type==BoundaryTypeV ? -boundary[IX(i,N)] : boundary[IX(i,N)];
	}
...
void Stable2DBoundaryStrategy::ExportClass()
{
	class_<Stable2DBoundaryStrategy, bases<BoundaryStrategy>>("Stable2DBoundaryStrategy", init<>())
		.def("setUpBoundary", &Stable2DBoundaryStrategy::setBoundary)
